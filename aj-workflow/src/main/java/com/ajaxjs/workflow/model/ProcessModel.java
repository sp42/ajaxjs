...
public class ProcessModel extends BaseModel {
	 ...
	 * @param nextModels 目标下一步模型集合
	 * @param clazz      指定要匹配的类型
	 */
	private static <T> void buildModels(List<T> models, List<T> nextModels, Class<T> clazz) {
		for (T nextModel : nextModels) {
			if (!models.contains(nextModel)) {// 如果 models 里面没有 nextModel 则添加之
