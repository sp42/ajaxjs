...
public class WorkflowUtils {
	 ...
	 * @return
	 */
	public static InputStream getStreamFromClasspath(String resource) {
		InputStream stream = Thread.currentThread().getContextClassLoader().getResourceAsStream(resource);

		if (stream == null)
			stream = WorkflowUtils.class.getClassLoader().getResourceAsStream(resource);

		if (stream == null)
			throw new WorkflowException("resource %s does not exist", resource);
	 ...
	 * @return 是否有数字和非零
	 */
	public static boolean hasNumber(Long l) {
		return l != null && l != 0;
	}
