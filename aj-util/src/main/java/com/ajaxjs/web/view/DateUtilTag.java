...
public class DateUtilTag extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		Date date = DateUtil.object2Date(value);

		if (date == null)
			return;

		String format = this.format == null ? DateUtil.formatDateShorter(date) : DateUtil.simpleDateFormatFactory(this.format).format(date);
