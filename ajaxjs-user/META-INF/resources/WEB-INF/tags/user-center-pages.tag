...
<c:if test="${type == 'avatar'}">
	...
	<form method="put" action="${ctx}/user/user-center/profile/" class="avatar" style="width:400px;margin:0 auto;text-align: center;">
		...
		<aj-xhr-upload action="${ctx}/user/user-center/profile/avatar/${userUid}/?id=${userId}" :is-img-upload="true" 
			hidden-field="avatar" 
		...
		<br />
		<br />
		<button class="aj-btn">保存</button> <button onclick="history.back();" class="aj-btn">返回</button>
	</form>
