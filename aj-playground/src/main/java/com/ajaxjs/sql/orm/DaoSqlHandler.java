...
public class DaoSqlHandler {
	...
	static ArgsInfo doSql(String sql, Method method, Object[] args) {
		...
		if (!ObjectUtils.isEmpty(args) && !ObjectUtils.isEmpty(parameters)) {
			String realSql = null;
	 ...
	 * @throws DaoException
	 */
	static String doSqlFactory(String sql, Method method, Class<?> daoClz) {
		...
		if (sqlFactoryHandler == null)
			throw new Error("找不到对应的 SqlFactory，应该为 Function<String, String>");
