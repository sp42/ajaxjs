...
import com.ajaxjs.util.Algorithm;
...
public class HttpSessionCookieStore {
	...
	public void deseriable (HttpSession session) {
		...
		for(Cookie cookie: cookies){
			if(cookie.getName().equals(sessionCookieName)){
				try {
					String value = Algorithm.Decrypt(cookie.getValue(), key);
	...
	public void seriable(HttpSession session) {
		...
		try {
			httpServletResponse.addCookie(getCookie(Algorithm.Encrypt(sb.toString(), key)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
