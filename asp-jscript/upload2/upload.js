...
$$.upload=function(cfg){
    ...
    $$.upload.prototype = {
                 setRawData : function(){
                    var rawData    =  Request.binaryRead(Request.totalBytes);
                    // http://social.technet.microsoft.com/Forums/pt-BR/5bb9ee94-59e7-4aba-bd1d-3be32a8b5d20/windows-2003-server-x64-iisasp?forum=1708
                    var binHolder  = new ActiveXObject('adodb.stream');
                        
                    // 设置一容器收集二进制数据，Byte []存放在adodb.stream对象中。
                        binHolder.open();
                        binHolder.type = 1;
                        binHolder.write(rawData);      
