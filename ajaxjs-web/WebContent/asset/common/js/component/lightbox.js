;(function(){
	window.bf_lightbox = Object.create(window.bf_tab);
	bf_lightbox.moverTagName = 'ul';
	bf_lightbox.isEnableLoop = false;
	bf_lightbox.isEnableTouch = true;
	bf_lightbox.isUsePx = false; 
	bf_lightbox.isGetCurrentHeight = false;
	bf_lightbox.isDirectShow = true;
	bf_lightbox.showing = false;
	// override
	bf_lightbox.go = function(i){
		var el = this.el;
		...
		this.isDirectShow && !this.showing && setTimeout(function(){
           ...
           bf_lightbox.showing = true;
        }, 500);
        
		location.setUrl_hash('picIndex', i);
		window.bf_tab.go.call(this, i, true);
	}
	// override
	bf_lightbox.init = function(){
		bf_tab.init.apply(this, arguments);
