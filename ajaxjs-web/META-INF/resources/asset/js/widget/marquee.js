;(function(re) {
	bf_marquee = {
		init : function(oScroller) {
			var listEl = oScroller.querySelector('div');

			this.boxHeight = oScroller.offsetHeight;
			...
			oScroller.appendChild(listEl.cloneNode(true));
			...
			if (this.PauseStep <= 0 || this.PauseHeight <= 0)
				this.PauseStep = this.PauseHeight = 0;
		...
		// 开始滚动
		Scroll : function() {
			var iScroll = this.oScroller.scrollTop, iHeight = this.listHeight, time = this.Time, iStep = this.Step * this.side;

			if (this.side > 0) {
				if (iScroll >= (iHeight * 2 - this.boxHeight))
					iScroll -= iHeight;
			} else {
				if (iScroll <= 0)
					iScroll += iHeight;
	...
	function autoPauseHeight(ul) {
		var li = ul.querySelector('li'), lineHeight = getComputedStyle(li).lineHeight;
