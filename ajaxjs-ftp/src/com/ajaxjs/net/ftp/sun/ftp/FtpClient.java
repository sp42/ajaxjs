...
public class FtpClient extends TransferProtocolClient {
	...
	public static boolean matchNonProxyHosts(String host) {
		synchronized (FtpClient.class) {
			String rawList = java.security.AccessController
					.doPrivileged(new sun.security.action.GetPropertyAction("ftp.nonProxyHosts"));
			if (rawList == null) {
				...
				if (!rawList.equals(nonProxyHostsSource)) {
					...
					try {
						...
						System.err.println("Error in http.nonProxyHosts system property: " + e);
					}
	...
	protected Socket openPassiveDataConnection() throws IOException {
		...
		if (proxy != null) {
			if (proxy.type() == Proxy.Type.SOCKS) {
				s = (Socket) AccessController.doPrivileged(new PrivilegedAction() {
					public Object run() {
