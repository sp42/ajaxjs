...
public class ProcessService extends AccessService implements IProcessService, CacheManagerAware {
	...
	public Process getProcessById(String id) {
		...
		if (entity != null) {
			if (org.snaker.engine.RzUtils.isDebugEnabled) {
				log.info(String.format("obtain process[id=%s] from database.", id));
			}
			cache(entity);
		}
		return entity;
	...
	public Process getProcessByVersion(String name, Integer version) {
		...
		if (entity != null) {
			if (org.snaker.engine.RzUtils.isDebugEnabled) {
				log.info(String.format("obtain process[name=%s] from cache.", processName));
			}
			return entity;
		}
		...
		if (processs != null && !processs.isEmpty()) {
			if (org.snaker.engine.RzUtils.isDebugEnabled) {
				log.info(String.format("obtain process[name=%s] from database.", processName));
			}
	...
	private void cache(Process entity) {
		...
		if (nameCache != null && entityCache != null) {
			...
			if (org.snaker.engine.RzUtils.isDebugEnabled) {
				log.info("no cache implementation class");
			}
		}
	}
