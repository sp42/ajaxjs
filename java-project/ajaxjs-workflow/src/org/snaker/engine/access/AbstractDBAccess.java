...
import org.snaker.engine.RzUtils;
...
public abstract class AbstractDBAccess implements DBAccess {
    private static final Logger log = Logger.getLogger(AbstractDBAccess.class.getName());
	...
	protected Dialect getDialect() {
		...
		if(dialect == null) {
			try {
				...
				log.warning("Unable to find the available dialect.Please configure dialect to snaker.xml");
			}
		}
		return dialect;
	}
	...
	public List<CCOrder> getCCOrder(String orderId, String... actorIds) {
		...
		if(actorIds != null && actorIds.length > 0) {
			...
			where.append(RzUtils.repeat("?,", actorIds.length));
			where.deleteCharAt(where.length() - 1);
			where.append(") ");
		}
		return queryList(CCOrder.class, where.toString(), RzUtils.add(actorIds, 0, orderId));
	}
    ...
    public <T> List<T> queryList(Page<T> page, QueryFilter filter, Class<T> clazz, String sql, Object... args) {
        ...
        if(org.snaker.engine.RzUtils.isDebugEnabled) {
            log.info("查询分页countSQL=\n" + countSQL);
            log.info("查询分页querySQL=\n" + querySQL);
        }
        try {
