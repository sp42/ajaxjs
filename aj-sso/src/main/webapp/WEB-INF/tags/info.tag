...
<%@ attribute fragment="false" required="false" name="date_style"
	type="Integer" description="1=创建；2=修改; 3=创建 && 修改"%>
<%@ attribute fragment="false" required="false" name="field_style"
	type="Integer" description="1=id；2=name; 4=content; 8 =state; 15 = all"%>
<%@ attribute fragment="false" required="false" name="two_cols"
	type="Boolean" description="表格是否双列"%>
<%
	...
	<script>
		function save() {
			...
			if(${isCreate}) {
				...
				jsp.xhr.putJson('/${ctx}/common_write?tableName=${TABLE_NAME}', json, j => {
