	...
	<div class="admin-entry-form">
		...
		<ajaxjs-admin-header> 
			...
			<template slot="btns"> <c:if test="${!isCreate}">
				<a :href="ajResources.ctx + '/admin/${shortName}/'">新建</a> | </c:if> <a :href="ajResources.ctx + '/admin/${shortName}/list/'">${uiName}列表</a> | 
			</template> 
		</ajaxjs-admin-header>
		<!-- // 后台头部导航 -->
		...
		<!-- 图片上传（iframe 辅助） -->
		<ajaxjs-fileupload-iframe upload-url="${ctx}/admin/${shortName}/upload/" ref="uploadIframe" accpect-file-type=".apk"></ajaxjs-fileupload-iframe>
