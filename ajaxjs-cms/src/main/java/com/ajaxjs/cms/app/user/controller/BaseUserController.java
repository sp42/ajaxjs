...
public abstract class BaseUserController extends CommonController<User, Long> {
	 ...
	 * @return
	 */
	public JsonReuslt sendSMScode(String phoneNo) {
		LOGGER.info("发送短信到::" + phoneNo);
		JsonReuslt json = null;

		if (phoneNo.length() != 11) {
			json = new JsonReuslt("手机号应为11位数", JsonReuslt.notOk);
		} else {
			if (CommonUtil.regMatch(phoneRegex, phoneNo) == null) {
				json = new JsonReuslt("非法手机号码", JsonReuslt.notOk);
			} else {
				...
				if (sms.send(message)) {
					...
					json = new JsonReuslt("发送手机 " + phoneNo + " 验证码成功", JsonReuslt.Ok);
				} else {
					json = new JsonReuslt("发送手机 " + phoneNo + " 验证码失敗", JsonReuslt.notOk);
				}
			}
		}
