...
public class UserPasswordFilter implements FilterAction {
	...
	public boolean before(MvcRequest request, MvcOutput response, Method method) {
		...
		UserCommonAuth auth = UserCommonAuthServiceImpl.dao.findByUserId(BaseUserController.getUserId());
		request.setAttribute("UserCommonAuthId", auth);

		if (auth.getPassword().equalsIgnoreCase(UserCommonAuthServiceImpl.encode(password))) {
			return true;
		} else {
