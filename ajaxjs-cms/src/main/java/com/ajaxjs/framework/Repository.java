...
//				sql = queryParam.orderToSql(sql);
	...
	private <R> Serializable insert(Insert insert, Method sqlFactoryHandler, Object[] args, Class<R> returnType) {
		...
		id = write(sql, sqlFactoryHandler, insert::tableName, 
				_sql -> create(conn, _sql, args), tableName -> createBean(conn, args[0], tableName), args);
	...
	private Integer update(Update update, Method sqlFactoryHandler, Object[] args) {
		...
		int effectRows = write(sql, sqlFactoryHandler, update::tableName, 
				_sql -> update(conn, _sql, args), tableName -> updateBean(conn, args[0], tableName), args);
		
		return effectRows;
	}
	
	private <T> T write(String sql, Method sqlFactoryHandler, 
			Supplier<String> getTableNameByAnn, Function<String, T> writeSql, Function<String, T> writeBean, 
			Object[] args) {
		T r = null;
	...
	private Boolean delete(Delete delete, Method sqlFactoryHandler, Object[] args) {
		...
		boolean isDeleted = write(sql, sqlFactoryHandler, delete::tableName, 
				_sql -> update(conn, _sql, args) >= 1, tableName -> delete(conn, args[0], tableName), args);
