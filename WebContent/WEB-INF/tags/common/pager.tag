<%@tag pageEncoding="UTF-8" description="分页UI"%>
<%@taglib uri="/ajaxjs" prefix="c"%>
<%@attribute name="pageInfo" type="com.ajaxjs.jdbc.PageResult" required="true" description="分页对象"%>
	...
	<c:choose>
		<c:when test="${pageInfo.totalCount > 0}">
			...
			<c:if
				...
				<a
					href="?start=${pageInfo.start + pageInfo.pageSize}${PageUtil.getParams_without('start', pageContext.request.queryString)}">下一页</a>
			</c:if>
