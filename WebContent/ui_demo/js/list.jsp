...
<body>
	...
	<ul class="list center">
		...
		<li>每行的结构由 tpl 决定</li>
		<li>可简单地设置 loading.在 HTML 直接设置，如 &lt;ul
			class=&quot;simpleList&quot;&gt; 加载中…… &lt;/ul&gt;</li>
		<li>该控件须引用 list.js</li>
		<li>方法签名：<pre class="prettyprint"
				style="height: 140px; width: 390px;">/**
 * 渲染一个列表
...
ajaxjs.List = function (url, el, args, cfg);</pre>
	...
	<ul class="center simpleList" style="width: 98%;">加载中……</ul>
	<script>
		ajaxjs.List(
						'../../public_service',
	...
	<ul class="center simpleList1"></ul>
	...
	<button class="ajaxjs-btn nextPageBtn2 center"style="display: block; margin-top: 10px;">下一页</button>
	<script>
		ajaxjs.List(
						'../../public_service',
		...
		<li>
			<a href="#">
				<div class="imgHolder noImg">
					<img
					data-src="<%=request.getParameter("style") != null && request.getParameter("style").equals("col3")
				? "[:=verticalPic:]"
				: "[:=horizontalPic:]"%>?w={0}"
					height="" />
					<div class="icon"></div>
	...
	<p>累加方式分页例子，isAppend = true</p>
	<ul class="simpleList3"></ul>
	<button class="nextPageBtn3 center" style="display: block; margin-top: 10px;">下一页</button>

	<script>
		ajaxjs.List('../../public_service', document.querySelector('ul.simpleList3'), {
			id : 2268,
			portalId : 45,
			start : 0,
			action : 'http_proxy',
	...
	<p>覆盖方式分页例子，isAppend = false</p>
	<ul class="center simpleList4">加载中……</ul>
	<button class="ajaxjs-btn nextPageBtn4 center" style="display: block; margin-top: 10px;">下一页</button>

	<script>
		ajaxjs.List(
						'../../public_service',
						document.querySelector('ul.simpleList4'),
						{
							id : 2268,
							portalId : 45,
							start : 0,
