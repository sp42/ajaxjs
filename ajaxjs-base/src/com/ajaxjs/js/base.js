...
Date.prototype.format = function(format) {
	...
	if (/(y+)/.test(format)) {
		$1 = RegExp.$1, format = format.replace($1, String(this.getFullYear())
				.substr(4 - $1));
	}

	for (key in o) { // 如果没有指定该参数，则子字符串将延续到 stringvar 的最后。
		if (new RegExp("(" + key + ")").test(format)) {
			$1 = RegExp.$1, value = String(o[key]),
					value = $1.length == 1 ? value : ("00" + value)
							.substr(value.length), format = format.replace($1,
							value);
		}
	}
	return format;
...
function java_value2js_value(v) {
	...
	if (v.getClass) {
		...
		default:
			...
			} else if (v[0].getClass() == java.lang.Integer
					|| v[0].getClass() == java.lang.Long) {
				var arr = [];
				for (var i = 0; i < v.length; i++) {
					arr.push(new Number(v[i]));
				}
				;
				return arr;
			} else
...
function pojo2json(pojo) {
	...
	for ( var k in pojo) {
		...
		if (hasGetter && k != 'getClass') {
			if (!pojo[k] || typeof (pojo[k]) != 'function')
				continute;
			try {
				value = pojo[k]();
			} catch (e) {
...
function singlePojo(pojo) {
	return JSON.stringify(pojo2json(pojo));
}
