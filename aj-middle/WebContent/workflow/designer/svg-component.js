;(function() {
	aj.svg.BaseComponent = {
		...
		created() {
			...
			aj.apply(this, {
				id: null,		// 每个图形对象赋予一个 id
				name: null,		// 通过 name 保存到 states 的 map 之中，它是 key
				svg: null,		// raphael 图形实例
				wfData: {},		// 工作流状态信息
				PAPER: null,	// 图形所在的桌布
				isDrag: true, 	// 是否启用鼠标拖放
				resize: true, 	// 是否启用放大缩小
				resizeNode:null,// resize 对象
				textNode: null	// 文字对象，这是 Vue 实例
			});
...
// 创建图形基类的工厂函数
aj.svg.createBaseComponent = function(PAPER, attr, type) {
	var raphaelObj;
	...
	var vueObj = new Vue({
		mixins: [aj.svg.BaseComponent],  data: {vBox: attr}
	});
