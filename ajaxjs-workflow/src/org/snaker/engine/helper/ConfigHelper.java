...
public class ConfigHelper {
	private static final transient Logger log = Logger.getLogger(ConfigHelper.class.getName());
	...
	public static void loadProperties(String filename) {
		...
		if (in == null) {
			...
			if (in == null) {
				log.warning("No properties file found in the classpath by filename " + filename);
			}
		} else {
			try {
				properties.load(in);
				...
				log.warning("Error reading from " + filename);
				e.printStackTrace();
			} finally {
				try {
					in.close();
				} catch (IOException e) {
					log.warning("IOException while closing InputStream: " + e.getMessage());
				}
			}
		}
	}
