...
import com.ajaxjs.mysql.model.MyDatabases;
...
public class DBInfoManager implements java.io.Serializable {
	...
	synchronized public DBInstanceInfo findDB(String groupName, String hostname) {
		DBGroupInfo cls = findGroup(groupName);
		if (cls == null)
			return null;
	...
	synchronized public boolean init() {
		...
		mydbs.clear();// refill
		List<DBInstanceInfo> dbList = null;
	...
	/**
	 * Will list all DBs and the dbs with user specified credentials are listed at
	 * the top
	 *
	 * @param user
	 * @return
	...
	/**
	 * This method will copy db credential when assign a DB to a restricted user or
	 * default admin/scanner user. Assume the assigner can access the underlying
	 * database.
	 *
	 ...
	 * @param password
	 */
	public void copyManagedDBCredential(String targetUserName, boolean restrictedUser, String srcUsername, String dbGroup, String dbuser, String password) {
