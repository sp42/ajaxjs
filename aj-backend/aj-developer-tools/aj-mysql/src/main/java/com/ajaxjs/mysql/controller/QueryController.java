...
import com.ajaxjs.mysql.db.CustomQueryProcessor;
import com.ajaxjs.mysql.db.InnoDbMutexPostProccessor;
import com.ajaxjs.mysql.db.MySQLStatusQueryProcessor;
import com.ajaxjs.mysql.db.PostQueryResultProcessor;
import com.ajaxjs.mysql.db.ReplLagQueryProcessor;
import com.ajaxjs.mysql.db.ReplShowProcessor;
import com.ajaxjs.mysql.db.TableMetaProcessor;
import com.ajaxjs.mysql.model.DBConnectionWrapper;
import com.ajaxjs.mysql.model.DBInstanceInfo;
import com.ajaxjs.mysql.model.QueryInputValidator;
...
import com.ajaxjs.mysql.snmp.SNMPQueryProcessor;
...
public class QueryController extends MyPerfBaseController {
	...
	private static Map<String, CustomQueryProcessor> CUSTOMER_PROCESSOR = new HashMap<String, CustomQueryProcessor>();

	private static Map<String, PostQueryResultProcessor> POST_PROCESSOR = new HashMap<String, PostQueryResultProcessor>();

	static {
		// current implementation requires the customized processor to be specified in
		// sql.xml
		CUSTOMER_PROCESSOR.put("TableMetaProcessor", new TableMetaProcessor());
		CUSTOMER_PROCESSOR.put("SNMPQueryProcessor", new SNMPQueryProcessor());
		CUSTOMER_PROCESSOR.put("ReplLagQueryProcessor", new ReplLagQueryProcessor());
		CUSTOMER_PROCESSOR.put("ReplShowProcessor", new ReplShowProcessor());
		CUSTOMER_PROCESSOR.put("MySQLStatusQueryProcessor", new MySQLStatusQueryProcessor());
	...
	protected ModelAndView handleRequestImpl(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		if (sql == null || sql.getQueryProcessor() == null || !CUSTOMER_PROCESSOR.containsKey(sql.getQueryProcessor())
		...
		// when we reach here, at least we have valid query and can connect to db if
		// needed
		WebAppUtil.storeLastDbInfoRequest(qps.getGroup(), qps.getHost(), req);

		try {
			logger.info("execute query " + qps.getSql() + ", " + qps.getSqlParams());
			if (sql != null && sql.getQueryProcessor() != null && CUSTOMER_PROCESSOR.containsKey(sql.getQueryProcessor())) // custom processor
			{
	...
	/**
	 * If filter condition is provided, filter the results Currently we only support
	 * single column based filtering with req parameter
	 * rf=col&rfv=value&rfv=value.... If provided, only result satisfying the
	 * condition (eq IN LIST) will be returned. For now, we allow case insensitive
	 * 
	 * @param rList
	 * @param req
	 */
