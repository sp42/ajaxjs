...
public class MetricsDefManager implements java.io.Serializable {
    ...
    synchronized public List<Metric> getMetrics(String grpName, String subGrpName, String[] ms) {
        ...
        } else mg = this.metricsGroups.get(grpName);
        ...
        for (String mname : ms) {
            if (mmap.containsKey(mname)) mList.add(mmap.get(mname).copy());
        }
        return mList;
    }
                ...
                if (mg != null) {
                    try {
                        while (reader.hasNext()) {
                            ...
                            if (evtType2 == XMLStreamConstants.END_ELEMENT && GROUP_TAG.equals(reader.getLocalName()))
                                break;
    ...
    private MetricsGroup parseSubGroup(XMLStreamReader reader) {
        ...
        if (mg != null) {
            try {
                while (reader.hasNext()) {
                    int evtType2 = reader.next();
                    if (evtType2 == XMLStreamConstants.END_ELEMENT && SUBGROUP_TAG.equals(reader.getLocalName())) break;
    ...
    private Metric parseMetric(XMLStreamReader reader) {
        ...
        if (mdbname == null || mdbname.isEmpty()) mdbname = pairs.get("name");
    ...
    private MetricsGroup parseMetricsGroupAttribute(XMLStreamReader reader) {
        ...
        if ("n".equalsIgnoreCase(isAuto)) mg.setAuto(false);
        else mg.setAuto(true);
        if (keyColumn != null && !keyColumn.isEmpty()) mg.setKeyColumn(keyColumn);
        return mg;
    }
