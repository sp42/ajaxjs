...
public class MetricsSubscribers {
		...
		public void deleteSubscription(Subscription sub) {
			...
			for (int i = this.subscriptions.size() - 1; i >= 0; i--) {
				...
				if (sub2.mGroup.equals(sub.mGroup)
						&& ((sub2.mSubGroup == null && sub.mSubGroup == null) || (sub2.mSubGroup != null && sub2.mSubGroup.equals(sub.mSubGroup)))) {
	...
	public MetricsSubscribers() {
		groupSubscriptions = new HashMap<>();
	}
	...
	public boolean addSubscription(Subscription sub) {
		...
		boolean status = this.context.getMetricDb() != null ? this.context.getMetricDb().addMetricsSubscription(sub) : false;
		if (!status)
			return false;
	...
	public boolean deleteSubscription(Subscription sub) {
		boolean status = this.context.getMetricDb() != null ? this.context.getMetricDb().deleteMetricsSubscription(sub) : false;
		if (!status)
			return false;
