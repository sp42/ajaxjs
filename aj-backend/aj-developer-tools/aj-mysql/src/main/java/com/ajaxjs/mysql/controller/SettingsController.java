...
import com.ajaxjs.mysql.config.MyPerfConfiguration;
...
public class SettingsController extends MyPerfBaseController {
	...
	private ModelAndView handleFetchSNMP(HttpServletRequest req, HttpServletResponse resp) {
		String dbgroup = req.getParameter("group");
		String host = req.getParameter("host");
		SNMPSetting snmpSetting = this.frameworkContext.getSnmpSettings().getHostSetting(dbgroup, host);
		ModelAndView mv = new ModelAndView(this.jsonView);
		String community = snmpSetting == null ? SNMPSetting.DEFAULT_COMMUNITY : snmpSetting.getCommunity();
		String ver = snmpSetting == null ? SNMPSetting.DEFAULT_VERSION : snmpSetting.getVersion();

		if (community == null || community.isEmpty())
			community = SNMPSetting.DEFAULT_COMMUNITY;
		if (ver == null || ver.isEmpty())
			ver = SNMPSetting.DEFAULT_VERSION;

		String enabled = snmpSetting == null ? "yes" : snmpSetting.getEnabled();
		StringBuilder sb = new StringBuilder();
		sb.append("{\"status\":0,\"message\":\"OK\", \"community\":\"").append(community).append("\",\"version\":\"").append(ver).append("\",\"enabled\":\"")
		...
		if ("3".equals(ver)) {
			...
			// logger.info("Ommit pwd: "+snmpSetting.getPassword()+",
			// "+snmpSetting.getPrivacyPassphrase());
		}
		sb.append("}");
	...
	private ModelAndView handleUpdateSNMP(HttpServletRequest req, HttpServletResponse resp) {
		boolean successful = this.frameworkContext.getSnmpSettings().updateSnmpSetting(req.getParameter("group"), req.getParameter("host"),
				req.getParameter("community"), req.getParameter("version"), req.getParameter("username"), req.getParameter("password"),
				req.getParameter("authprotocol"), req.getParameter("privacypassphrase"), req.getParameter("privacyprotocol"), req.getParameter("context"),
	...
	private ModelAndView handleUpdate(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(req.getParameter("metricsDbType"))) {
				message = this.testMetricsDB(req.getParameter("metricsDbHost"), req.getParameter("metricsDbPort"), req.getParameter("metricsDbName"),
						req.getParameter("metricsDbUserName"), req.getParameter("metricsDbPassword"), config);
			...
			if (alertScanInterval < 60)
				alertScanInterval = 300;
			...
			// hipchat
			{
				...
				if ((hipchatUrl == null && config.getHipchatUrl() != null) || (hipchatUrl != null && !hipchatUrl.equals(config.getHipchatUrl())))
					hipchatChanged = true;
				...
				if (hipchatAuthToken != null)
					hipchatAuthToken = hipchatAuthToken.trim();
