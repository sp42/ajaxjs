...
public class MySQLMetricsDb extends MetricsDbBase implements java.io.Serializable, Runnable {
	...
	// private java.text.SimpleDateFormat sdf = new
	...
	protected void storeGenericMetric(String s, List<MetricsData> q2, Connection conn) throws SQLException {
		Statement stmt = null;
		MetricsGroup mg = metricsGroups.get(s);
	...
	protected String[] buildAlertSettingDDL() {
		return new String[] { "create table " + MetricsDbBase.ALERTSETTING_TABLENAME
				+ "(DBGROUPNAME VARCHAR(30), HOSTNAME VARCHAR(100), ALERT_TYPE VARCHAR(30), THRESHOLD DECIMAL(22,7), RESERVED VARCHAR(255), CREATED TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(DBGROUPNAME, HOSTNAME, ALERT_TYPE))" };
	}

	@Override
	protected String[] buildSnapshotDDL() {
		return new String[] {
				"create table SNAPSHOTS(SNAP_ID INT NOT NULL AUTO_INCREMENT, START_TS BIGINT, END_TS BIGINT, FAILED_HOSTS INT, SUCCEED_HOSTS INT, CREATED TIMESTAMP DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(SNAP_ID),  KEY IDX_SNAPSHOTS_TS (START_TS))" };
	}
