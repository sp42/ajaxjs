...
public class MyPerfConfiguration implements java.io.Serializable {
    ...
    public void init(MyPerfContext ctx) {
        ...
        } else cfgFile = new File(myperfConfigPath);
        ...
        if (readConfig(cfgFile, ctx)) configured = true;
    ...
    synchronized public boolean store(MyPerfContext ctx) {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(cfgFile))) {
            ...
            if (this.metricsDbPort > 0) pw.println("metricsDbPort=" + this.metricsDbPort);
            ...
            if (this.hipchatUrl != null && !this.hipchatUrl.isEmpty()) pw.println("hipchatUrl=" + this.hipchatUrl);
