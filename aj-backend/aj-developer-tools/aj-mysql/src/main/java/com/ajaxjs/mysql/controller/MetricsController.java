...
import com.ajaxjs.mysql.common.WebAppUtil;
import com.ajaxjs.mysql.model.ColumnDescriptor;
import com.ajaxjs.mysql.model.ColumnInfo;
import com.ajaxjs.mysql.model.CustomResultObject;
...
import com.ajaxjs.mysql.model.MetricsResultObject;
...
public class MetricsController extends MyPerfBaseController {
	...
	/**
	 * URL path pattern:
	 * /metrics/{cmd}/{dbgroup}/{dbhost}/[{metrics}/[{start_ts}/][{end_ts}/]]{random}.html
	 * cmd: get: get one or more metrics for one dbhost. metrics name separated by
	 * comma
	 *
	 * @param req
	 * @return
	...
	private void prefixMetricGroupName(String mg, ResultList rs) {
		if (mg == null || "globalstatus".equals(mg) || mg.isEmpty() || rs == null)
			return;
		for (ColumnInfo col : rs.getColumnDescriptor().getColumns()) {
			String name = col.getName();
			if ("TS".equals(name) || "SNAP_ID".equals(name))
				continue;
	...
	private ModelAndView getMetrics(Map<String, String> pathParameters, HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		do {
			if (dbinfo == null) {
				...
				message = "There is no record for DB (" + pathParameters.get(Constants.URL_PATH_DBGROUP) + ", " + pathParameters.get(Constants.URL_PATH_DBHOST)
						+ "), please add it to the system first.";
				break;
			}
			...
			for (String s : ms) { // all name should be in the form of {group}.{subgrp}.{metric name}
				String[] cmp = s.split("\\."); // retrieve name component
				if (cmp.length != 3)
					continue; // invalid name, ignore
				if (cmp[2] == null || cmp[2].isEmpty())
					continue;// invalid name, ignore
				String mg = cmp[0];
			...
			for (String mg : metricsByGroup.keySet()) {
				...
				if (mg.startsWith("UDM_")) {
					try {
						mGroup = this.frameworkContext.getMetricsDef().getUdmManager().getUDMByName(mg.substring(mg.indexOf('_') + 1)).getMetricsGroup();
					} catch (Exception ex) {
					}
				} else
				...
				for (String subGrpName : subGroups.keySet()) {
					...
					if (mGroup != null && mGroup.isStoreInCommonTable()) {
						...
						tmpList = retrieveMetricsWithKey(subGroups.get(subGrpName).toArray(new String[0]), mg, subGrpName, dbinfo, dateRange[0], dateRange[1],
								true);
					else
					...
					if (rlist == null && tmpList != null) {
						...
						rlist = this.mergeResultList(rlist, tmpList);
					if (rlist != null)
		...
		if (status == Constants.STATUS_BAD) {
			...
			mv.addObject("json_result",
					useKeyColumn
							? ResultListUtil.toMetricsJSONStringWithMultiRowsKeys(rlist, "KEY_COLUMN", new String[] { "SNAP_ID", "TS" }, metricsList[0], qps,
									status, message)
	...
	protected ModelAndView handleRequestImpl(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		return this.respondFailure(
				"Invalid command. Supported URL: /metrics/{enable, disable,get}/{dbgroup}/{dbhost}/{metrics_separated_by_comma}/[{start_time}]/[{end_time}]/{random_number}.html",
				req);
	}
	 ...
	 * @return
	 */
	private ResultList retrieveMetricsWithKey(String[] metrics, String metricgroup, String subGroup, DBInstanceInfo dbinfo, String startDate, String endDate,
			boolean agg) {
		ResultList rList = null;
		try {
			String tblName = subGroup;
			if (tblName == null || tblName.isEmpty() || tblName.equals("_"))
				tblName = metricgroup;
			if (metricgroup.startsWith("UDM_"))
				tblName = metricgroup;
			else
				tblName = "MYSQL_" + tblName;

			List<Metric> mtrs = this.frameworkContext.getMetricsDef().getMetrics(metricgroup, subGroup, metrics);
			long sqlStartTime = System.currentTimeMillis();
			rList = this.frameworkContext.getMetricDb().retrieveMetrics(tblName.toUpperCase(), mtrs.toArray(new Metric[0]), true, dbinfo.getDbid(),
					Long.parseLong(startDate), Long.parseLong(endDate), agg);
		...
		try {
			{
				...
				if (metricgroup.startsWith("UDM_"))
					...
					tblName = "MYSQL_" + tblName;
				long sqlStartTime = System.currentTimeMillis();
				rList = this.frameworkContext.getMetricDb().retrieveMetrics(tblName.toUpperCase(), metrics, false, dbinfo.getDbid(), Long.parseLong(startDate),
	...
	private ResultList retrieveGenericMetrics(String[] metrics, MetricsGroup mg, DBInstanceInfo dbinfo, String startDate, String endDate) {
		...
		for (String mname : metrics) {
			try {
				...
				if (metricFullName == null)
					continue;
				ResultList tmpList = this.frameworkContext.getMetricDb().retrieveUDMMetrics(metricFullName, dbinfo.getDbid(), Long.parseLong(startDate),
						Long.parseLong(endDate));
				if (tmpList == null)
					continue;
				Metric m = mg.getMetricByName(mname).copy();
				m.setName(metricFullName);
				List<Metric> mtrs = new ArrayList<Metric>(1);
				mtrs.add(m);
				MetricsResultObject mrs = new MetricsResultObject();
				mrs.setMetrics(mtrs);
	...
	private ResultList mergeResultList(ResultList rList1, ResultList rList2) {
		...
		// we have use the same timestamp and data are sorted, ignore if we have any
		// missing data in one list
		int idx1 = 0;
		int idx2 = 0;
