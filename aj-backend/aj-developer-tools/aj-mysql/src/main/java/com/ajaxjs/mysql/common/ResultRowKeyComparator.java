...
/**
 * Compare two ResultRows based on keyList. For simplicity, the column
 * descriptors for both rows should be provided
 * 
 * @author xrao
 *
 */
public class ResultRowKeyComparator implements Comparator<ResultRow>, java.io.Serializable {
	...
	public ResultRowKeyComparator(List<String> keyList, ColumnDescriptor descA, ColumnDescriptor descB) {
		...
		this.index1List = new java.util.ArrayList<>(this.keyList.size());
		this.index2List = new java.util.ArrayList<>(this.keyList.size());
