...
public class AlertScannerRunner implements Runnable {
	...
	public AlertScannerRunner(MyPerfContext frameworkContext, LinkedBlockingQueue<DBInstanceInfo> dbqueue, AppUser appUser) {
	...
	private void scan() {
		...
		while (!dbqueue.isEmpty()) {
			try {
				...
				scanHost(db);
				// this.frameworkContext.getInstanceStatesManager().getStates(db.getDbid()).setLastScanTime(System.currentTimeMillis()
				// - startTime);
			} catch (Exception ex) {
	...
	private boolean scanHost(DBInstanceInfo dbinfo) {
		...
		try {
			...
			List<Subscription> subscripts = this.frameworkContext.getMetricsDef().getUdmManager().getAlertSubscriptions()
					.getSubscriptions(dbinfo.getDbGroupName(), dbinfo.getHostName());
			...
			for (Subscription subscript : subscripts) {
	...
	private void scanAlert(ScanData scanData, Subscription subscript) {
		...
		try {
			...
			if (rList != null && rList.getRows().size() > 0) {
				if (ist.reportUserAlert(new Date(startTime), def.getName(), String.valueOf(rList.getRows().size()))) {
					AlertEntry alertEntry = new AlertEntry(startTime, subscript.alertName, null, scanData.dbinfo.getDbGroupName(),
					...
					this.frameworkContext.getMetricDb().addNewAlert(Long.parseLong(sdf.format(startTime)), scanData.dbinfo.getDbid(), subscript.alertName,
							null);
				}
				hasAlert = true;
			} else {
				if (ist != null && ist.getLastAlertType() != null && ist.getLastAlertType().equals(def.getName())) {
					this.frameworkContext.getMetricDb().markAlertEnd(Long.parseLong(sdf.format(startTime)) - 100, scanData.dbinfo.getDbid(),
							System.currentTimeMillis(), subscript.alertName);
				}
			}
