...
/**
 * User management Note the usage of model value a: 0 self for user management
 * from login user 1 for self sign up 2 action message after self signup/pwd
 * reset 3 for self reset
 *
 * @author xrao
 */
public class UserController extends MyPerfBaseController {
    ...
    protected ModelAndView handleRequestImpl(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if (appUser == null && "new".equalsIgnoreCase(actionSrc)) {
            ...
            // form
            if (name == null || name.isEmpty()) return this.showSelfSignupForm(req);
            // signup
            return this.handleSelfSignup(req);
        }

        // password reset, form and action
        if ("reset".equalsIgnoreCase(actionSrc)) return handleReset(req);
...
//					"The administrator has confirmed your user account. It is time to use it and enjoy it.");
    ...
    private ModelAndView handleChangePrivilege(HttpServletRequest req) {
        ...
        if (name == null || name.equalsIgnoreCase(appUser.getName())) {
            ...
            try {
                p = Integer.parseInt(userprivilege);
            } catch (Exception ex) {
            }
...
//						"There is a pending user request, with user name " + newUser.getName() + ", email address " + newUser.getEmail());
                message = "Your requets has been sent to administrator. If you do not recieve confirmation on time, please contact administrator by email " + this.frameworkContext.getMyperfConfig().getAdminEmail();
                mv.addObject("a", 2);
...
//				}
    ...
    private ModelAndView retrieveUserInfo(HttpServletRequest req) {
        ...
        else if (!appUser.isAdminUser()) name = appUser.getName();
    ...
    private ModelAndView retrieveUserList(HttpServletRequest req) {
        ...
        for (AppUser u : users) {
            if (appUser.getName().equals(u.getName())) continue;
            ResultRow row = new ResultRow();
    ...
    private ModelAndView retrieveNewUserList(HttpServletRequest req) {
        ...
        for (AppUser u : users) {
            if (appUser.getName().equals(u.getName()) || u.isVerified()) continue;
            ResultRow row = new ResultRow();
            row.setColumnDescriptor(desc);
            row.addColumn(u.getName());
            rs.addRow(row);
    ...
    private ModelAndView listAllUsers(HttpServletRequest req) {
        ...
        for (AppUser u : users) {
            ...
            if (u.isAdminUser()) userType = "Power User";
            else if (u.isRestrictedUser()) userType = "Restricted User";
    ...
    private void appendUserListToModel(HttpServletRequest req, ModelAndView mv) {
        ...
        for (AppUser u : userList) {
            if (!u.getName().equalsIgnoreCase(appUser.getName())) allUsers.add(u.getName());
            if (!u.isVerified()) newUsers.add(u.getName());
        }
