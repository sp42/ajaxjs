...
/**
 * This records configuration history. It includes: a starting ConfigBlock with
 * all configuration variables for the first time it is tracked a series of
 * changes of ConfigBlock last checked configBlock
 *
 * @author xrao
 */
@Data
public class ConfigHistory implements java.io.Serializable {
    ...
    private static Map<String, Integer> EXCLUDE_ENTRIES = new HashMap<>();

    private ConfigBlock startingConfig;
    private ConfigBlock lastCheckedConfig;
    private List<ConfigBlock> changes = new ArrayList<>();
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.isEmpty()) {
                    ...
                    if (idx < line.length() - 1) cb.addVariable(line.substring(0, idx), line.substring(idx + 1));
                    else cb.addVariable(line.substring(0, idx), "");
                }
            }
        } finally {
            if (br != null) try {
    ...
    public void write(OutputStream out) throws Exception {
        ...
        try {
            ...
            // let's start with changes and only leave first and last as references at the
            // end
            for (ConfigBlock cb : this.changes) {
                bw.println("[CHANGES]");
                bw.println("[" + cb.getTime() + "]");
                for (Map.Entry<String, String> e : cb.getVariables().entrySet()) {
                    if (e.getValue() != null) bw.println(e.getKey() + "=" + e.getValue());
                    else bw.println(e.getKey() + "=");
                }
    ...
    public boolean updateLast(ConfigBlock last) {
        ...
        for (Map.Entry<String, String> e : lastMap.entrySet()) {
            ...
            if (!prevMap.containsKey(key)) diffMap.put(key, val);
            else {
        ...
        for (Map.Entry<String, String> e : prevMap.entrySet()) {
            ...
            String val = e.getValue();
            if (EXCLUDE_ENTRIES.containsKey(key)) continue;
            if (!lastMap.containsKey(key)) diffMap.put("-" + key, val);// use - to record a variable is removed
        }

        if (diffMap.size() == 0) return false;
