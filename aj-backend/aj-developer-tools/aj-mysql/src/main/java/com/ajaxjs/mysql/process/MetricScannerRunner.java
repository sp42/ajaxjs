...
import com.ajaxjs.mysql.common.Constants;
import com.ajaxjs.mysql.common.DBUtils;
import com.ajaxjs.mysql.config.MyPerfContext;
import com.ajaxjs.mysql.db.InnoDbMutexPostProccessor;
...
import com.ajaxjs.mysql.model.MetricsBuffer;
import com.ajaxjs.mysql.model.MetricsGroup;
...
import com.ajaxjs.mysql.model.StateSnapshot;
import com.ajaxjs.mysql.model.UserDefinedMetrics;
import com.ajaxjs.mysql.model.alert.AlertDefinition;
import com.ajaxjs.mysql.model.alert.AlertEntry;
import com.ajaxjs.mysql.model.alert.AlertSettings;
import com.ajaxjs.mysql.model.alert.subscription.Subscription;
import com.ajaxjs.mysql.snmp.SNMPClient;
import com.ajaxjs.mysql.snmp.SNMPSetting;
...
public class MetricScannerRunner implements Runnable {
	...
	public MetricScannerRunner(MyPerfContext frameworkContext, LinkedBlockingQueue<DBInstanceInfo> dbqueue, AppUser appUser, int snap_id) {
	...
	private void checkAndReportDiskUsageAlert(DBInstanceInfo dbinfo, String diskMount, long value) {
		...
		if (threshold == null || (float) value <= threshold) {
			// logger.info("Disk usage is under control: mount - "+diskMount+", used -
			// "+value+", threshold - "+threshold);
			return;
		}
		try {
			Date dt = new Date();
			if (dbinfo.isAlertEnabled() && this.frameworkContext.getInstanceStatesManager().getStates(dbinfo.getDbid()).reportUserAlert(dt, "DISKUSAGE",
					diskMount + " " + String.valueOf(value) + "%")) {
	...
	private void scanSnmpDisk(ScanData scanData, MetricsGroup mg) throws Exception {
		if (scanData.dbinfo.isSnmpEnabled() && scanData.canQuerySNMP) {
			...
			try {
				...
				snmpClient.setSnmpSetting(
						this.frameworkContext.getSnmpSettings().getHostSetting(scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName()));
				snmpClient.start();
	...
	private MetricsBuffer retrieveMetricsBuffer(ScanData scanData, MetricsGroup mg, MetricsGroup subGrp) {
		...
		if (mg.isUdmFlagged())// we have to deal with UDM on the fly
		{
			...
			return buffer.get(scanData.dbinfo.getDbGroupName() + ":" + scanData.dbinfo.getHostName()).get(mg.getGroupName());
		else
			return buffer.get(scanData.dbinfo.getDbGroupName() + ":" + scanData.dbinfo.getHostName()).get(mg.getGroupName() + "." + subGrp.getGroupName());
	...
	private void storeKVTabularDataGeneric(ScanData scanData, MetricsGroup mg, Map<String, String> kvPairs, int queryTime) throws SQLException {
		...
		for (Metric m : mg.getMetrics()) {
			...
			if (!kvPairs.containsKey(srcName))
				continue;
			int metricId = this.frameworkContext.getAutoScanner().getMetricDb().checkAndAddMetricCode(mg.getMetricFullName(m.getName()));
			if (metricId <= 0) {
				logger.warning("Cannot find metric id for " + m.getName() + ", " + mg.getMetricFullName(m.getName()));// TODO cache local
				break;// stop here
			}
			try {
				...
				int pos = 0;
				buf2.putInt(pos, scanData.dbinfo.getDbid());
				pos += 4;
				buf2.putInt(pos, metricId);
				pos += 4;
				buf2.putInt(pos, scanData.snap_id);
				pos += 4;
				buf2.putLong(pos, scanData.startTimestamp);
				pos += 8;
	...
	private void storeKVTabularData(ScanData scanData, MetricsGroup mg, String key, Map<String, String> kvPairs, int queryTime) throws SQLException {
		...
		if (mg.isStoreInCommonTable()) {
			storeKVTabularDataGeneric(scanData, mg, kvPairs, queryTime);
			return;
		}
		...
		if (mg.getSubGroups() != null && mg.getSubGroups().size() > 0) {
			for (MetricsGroup subGrp : mg.getSubGroups()) {
				...
				if (!subGrp.isAuto()) {
					if (!mg.isAuto()) { // Skip groups requiring manual configuration for now
						if (!this.frameworkContext.getMetricsDef().getUdmManager().isMetricsGroupSubscribed(scanData.dbinfo.getDbGroupName(),
								scanData.dbinfo.getHostName(), mg.getGroupName(), subGrp.getGroupName()))
							continue;
					}
				}
				...
				if (mbuf != null) {
					...
					if (buf != null)
						this.frameworkContext.getMetricDb().putData(subGrp, key, buf);
				}
			}
		} else {
			...
			if (mbuf != null) {
				ByteBuffer buf = mbuf.recordOneRowByMetricsMap(kvPairs, scanData.snap_id, scanData.startTimestamp, queryTime);
				if (buf != null)
					this.frameworkContext.getMetricDb().putData(mg, key, buf);
			}
		}
		// TODO attached UDM
		List<String> attachedUDMs = this.frameworkContext.getMetricsDef().getUdmManager().getUDMsAttachedToBuiltinMetrics(mg.getGroupName(),
				scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName());
		if (attachedUDMs == null || attachedUDMs.size() == 0)
			return;
		for (String s : attachedUDMs) {
			UserDefinedMetrics udm = this.frameworkContext.getMetricsDef().getUdmManager().getUDMByName(s);
			if (udm == null || udm.getMetricsGroup() == null)
				continue;
			...
			if (mbuf != null) {
				ByteBuffer buf = mbuf.recordOneRowByMetricsMap(kvPairs, scanData.snap_id, scanData.startTimestamp, queryTime);
				if (buf != null)
					this.frameworkContext.getMetricDb().putData(udmG, key, buf);
			}
		}
	}
	...
	private void recordUserAlertsFromMetricsData(ScanData scanData, MetricsGroup mg, Map<String, String> kvPairs) {
		if (kvPairs == null)
			return;
		String mgName = "";
		if (mg.getParentGroup() != null)
			mgName = mg.getParentGroup() + ".";
		else if (mg.isUdmFlagged())
			mgName = "UDM.";
		mgName += mg.getGroupName();
		List<Subscription> subs = this.frameworkContext.getMetricsDef().getUdmManager().getAlertsAttachedToBuiltinMetrics(mgName,
				scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName());
	...
	private void recordUserAlertsFromGlobalStatus(ScanData scanData, Map<String, String> kvPairs) {
		...
		List<Subscription> subs = this.frameworkContext.getMetricsDef().getUdmManager().getAlertsAttachedToGlobalStatus(scanData.dbinfo.getDbGroupName(),
		...
		for (Subscription sub : subs) {
			...
			if (mname != null && kvPairs.containsKey(mname)) {
				...
				scanData.stateSnap.addMetric(def.getName(), f);
				// logger.info("Global status alert data: "+def.getName()+", metrics: "+mname+",
				// value: " + f);
			}
		}
	}
	...
	private void scanRegularPredefined(ScanData scanData, MetricsGroup mg) throws Exception {
		// logger.info("Scan group "+mg.getGroupName()+" for "+scanData.dbinfo+", udm:
		// "+mg.isUdmFlagged());
		// The idea to use sub group is, we can scan once and update multiple sub
		// groups.
		QueryParameters qps = new QueryParameters();
		if (mg.isUdmFlagged())
		...
		if (mg.getKeyColumn() == null || mg.getKeyColumn().isEmpty()) {
			...
			if ("mysql_global_status".equalsIgnoreCase(mg.getSql())) {
				...
				kvPairs = this.frameworkContext.getQueryEngine().executeQueryWithKeyValuPairs(qps, scanData.conn, mg.getMetricNameColumn(),
						mg.getMetricValueColumn(), false);
			...
			Map<String, Map<String, String>> mkvPairs = this.frameworkContext.getQueryEngine().executeQueryWithMultipleColumns(qps, scanData.conn,
					mg.getKeyColumn());
	...
	private boolean scanHost(DBInstanceInfo dbinfo) {
		...
		try {
			...
			for (String mgName : mgNames) {
				...
				if (!mg.isAuto()) { // skip groups requiring manual configuration for now
					if (!this.frameworkContext.getMetricsDef().getUdmManager().isMetricsGroupSubscribed(dbinfo.getDbGroupName(), dbinfo.getHostName(),
							mg.getGroupName(), null))
						continue;
				}
				SNMPSetting snmpSetting = this.frameworkContext.getSnmpSettings().getHostSetting(dbinfo.getDbGroupName(), dbinfo.getHostName());
			...
			// UDM
			List<String> udms = this.frameworkContext.getMetricsDef().getUdmManager().getMetricsSubscriptions().getSubscribedUDMs(dbinfo.getDbGroupName(),
					dbinfo.getHostName());
			if (udms != null) {
				for (String s : udms) {
					UserDefinedMetrics udm = this.frameworkContext.getMetricsDef().getUdmManager().getUDMByName(s);
			...
			// TODO handle alerts
			if (!scanData.statusUpdated && (scanData.stateSnap.getConnections() >= 0L || scanData.stateSnap.getThreads() > 0))// at least we get something from
																																// scan
			{
				InstanceStates ist = this.frameworkContext.getInstanceStatesManager().getStates(dbinfo.getDbid());
				if (ist != null) {
					Date lastAlertTime = ist.getLastAlertTime();
					Date lastAlertEndTime = ist.getLastAlertEndTime();
					String lastAlertType = ist.getLastAlertType();

					ist.update(scanData.stateSnap, this.getAlertThreshold(dbinfo));
					// TODO
					List<AlertEntry> userDefinedAlerts = ist.checkAndRaiseUserAlerts(this.frameworkContext, scanData.dbinfo.getDbGroupName(),
							scanData.dbinfo.getHostName());
					if (userDefinedAlerts != null && userDefinedAlerts.size() > 0 && dbinfo.isAlertEnabled()) {
						for (AlertEntry alert : userDefinedAlerts) {
							if (ist.reportUserAlert(new Date(alert.getTs()), alert.getAlertReason(), alert.getAlertValue())) {
								this.frameworkContext.getMetricDb().addNewAlert(Long.parseLong(sdf.format(alert.getTs())), dbinfo.getDbid(),
										alert.getAlertReason(), alert.getAlertValue());
								this.frameworkContext.addAlert(alert);
								this.frameworkContext.emailAlert(alert);
							}
						}
					...
					// if(lastAlertTime!=null &&( (lastAlertEndTime==null &&
					// ist.getLastAlertEndTime()!=null)
					// ||(ist.getLastAlertEndTime()==null && ist.getLastAlertTime()!=null &&
					// !ist.getLastAlertType().equalsIgnoreCase(lastAlertType))))
					if ((lastAlertTime != null && lastAlertEndTime == null) // existing an old alert
						...
						// if(!scanData.replDown)
						{
							...
							this.frameworkContext.getMetricDb().markAlertEnd(Long.parseLong(sdf.format(lastAlertTime.getTime())), dbinfo.getDbid(),
									Long.parseLong(sdf.format(scanData.stateSnap.getTimestamp())));
							// mark anything one minute ago, too
							// this.frameworkContext.getMetricDb().markAlertEnd(Long.parseLong(sdf.format(scanData.stateSnap.getTimestamp()))
							// - 100, dbinfo.getDbid(),
							// Long.parseLong(sdf.format(scanData.stateSnap.getTimestamp())));
						...
						ist.updateAlert(false, new Date(scanData.startTimestamp), scanData.snapAlertType, scanData.snapAlertValue, false);
					}
					if (ist.getLastAlertTime() != null && ist.getLastAlertEndTime() == null) {
						...
						if ((!"IO".equalsIgnoreCase(ist.getLastAlertType()) && !"SLOW".equalsIgnoreCase(ist.getLastAlertType())
								&& !"REPLLAG".equalsIgnoreCase(ist.getLastAlertType()) && !"REPLDOWN".equalsIgnoreCase(ist.getLastAlertType())
								&& !"MAXCONNERR".equalsIgnoreCase(ist.getLastAlertType())) || ist.getLastUpdateTime() == ist.getLastAlertTime().getTime()
			...
			if (scanData.conn != null) {
				conns.checkinConnectionAndClose(scanData.conn);
				scanData.conn = null;
			}
		}
		logger.fine("Done scan for host (" + dbinfo + ")");
		if (!scanData.statusUpdated && (scanData.stateSnap.getSyscputime() >= 0L || scanData.stateSnap.getThreads() > 0))
	...
	private void recordAlertReport(DBConnectionWrapper conn, DBInstanceInfo dbinfo, long timestamp, String alertType, String alertValue) {
		...
		if (this.frameworkContext.getInstanceStatesManager().getStates(dbinfo.getDbid()).canRunReport()) {
			...
			// this.frameworkContext.getAlerts().addAlert(new AlertEntry(timestamp,
			// alertType, alertValue, dbinfo.getDbGroupName(), dbinfo.getHostName()));
