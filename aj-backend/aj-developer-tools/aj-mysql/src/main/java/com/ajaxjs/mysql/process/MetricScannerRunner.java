...
public class MetricScannerRunner implements Runnable {
    ...
    private void checkAndReportDiskUsageAlert(DBInstanceInfo dbinfo, String diskMount, long value) {
        ...
        try {
            Date dt = new Date();
            if (dbinfo.isAlertEnabled() && this.frameworkContext.getInstanceStatesManager().getStates(dbinfo.getDbid()).reportUserAlert(dt, "DISKUSAGE", diskMount + " " + String.valueOf(value) + "%")) {
    ...
    private void scanSnmpDisk(ScanData scanData, MetricsGroup mg) throws Exception {
        if (scanData.dbinfo.isSnmpEnabled() && scanData.canQuerySNMP) {
            ...
            try {
                ...
                snmpClient.setSnmpSetting(this.frameworkContext.getSnmpSettings().getHostSetting(scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName()));
                snmpClient.start();
        ...
        if (scanData.dbinfo.isSnmpEnabled() && scanData.canQuerySNMP) {
            ...
            try {
                ...
                for (Map.Entry<String, List<SNMPClient.SNMPTriple>> entry : diskStats.entrySet()) {
                    ...
                    if ("Cached memory".equals(key) || "Memory buffers".equals(key) || "Physical memory".equals(key) || "/dev/shm".equals(key) || "Swap space".equals(key) || "Virtual memory".endsWith(key))
                        continue;
                    Map<String, String> snmpRes = new HashMap<>();
                    long diskUsed = 0L;
                    long diskSize = 0L;
                    for (SNMPClient.SNMPTriple e : entry.getValue()) {
                        ...
                        try {
                            ...
                            if ("HRSTORAGEUSED".equalsIgnoreCase(e.name)) diskUsed = lval;
    ...
    private void scanSnmpSys(ScanData scanData, MetricsGroup mg) throws Exception {
        if (scanData.dbinfo.isSnmpEnabled() && scanData.canQuerySNMP) {
            ...
            try {
                ...
                try {
                    if (snmpClient != null) snmpClient.stop();
                } catch (Exception snmpExc) {
                }
            }
        }
    ...
    private MetricsBuffer retrieveMetricsBuffer(ScanData scanData, MetricsGroup mg, MetricsGroup subGrp) {
        ...
        if (!buffer.containsKey(scanData.dbinfo.getDbGroupName() + ":" + scanData.dbinfo.getHostName())) return null;
    ...
    private void storeKVTabularData(ScanData scanData, MetricsGroup mg, String key, Map<String, String> kvPairs, int queryTime) throws SQLException {
            ...
            for (MetricsGroup subGrp : mg.getSubGroups()) {
                ...
                if (!subGrp.isAuto()) {
                    if (!mg.isAuto()) { // Skip groups requiring manual configuration for now
                        if (!this.frameworkContext.getMetricsDef().getUdmManager().isMetricsGroupSubscribed(scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName(), mg.getGroupName(), subGrp.getGroupName()))
                            continue;
                    }
                }
        ...
        // TODO attached UDM
        List<String> attachedUDMs = this.frameworkContext.getMetricsDef().getUdmManager().getUDMsAttachedToBuiltinMetrics(mg.getGroupName(), scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName());
    ...
    private void recordUserAlertsFromMetricsData(ScanData scanData, MetricsGroup mg, Map<String, String> kvPairs) {
        ...
        List<Subscription> subs = this.frameworkContext.getMetricsDef().getUdmManager().getAlertsAttachedToBuiltinMetrics(mgName, scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName());
    ...
    private void recordUserAlertsFromGlobalStatus(ScanData scanData, Map<String, String> kvPairs) {
        ...
        List<Subscription> subs = this.frameworkContext.getMetricsDef().getUdmManager().getAlertsAttachedToGlobalStatus(scanData.dbinfo.getDbGroupName(), scanData.dbinfo.getHostName());
    ...
    private void scanRegularPredefined(ScanData scanData, MetricsGroup mg) throws Exception {
        ...
        // groups.
        QueryParameters qps = new QueryParameters();
        if (mg.isUdmFlagged()) qps.setSqlText(mg.getSqlText());
        else qps.setSql(mg.getSql());
    ...
    private boolean tryConnect(ScanData scanData) {
        ...
        try {
            ...
            if (msg != null && (msg.contains("Access denied") || Constants.CONN_MSG_NORETRY.equals(msg))) {
        ...
        {
            try {
                logger.info("Retry to connect to " + scanData.dbinfo + ", last try time " + connTime + "(" + connStartTime + ", " + connEndTime + ")" + ", timeout " + this.frameworkContext.getConnectionTimeout());
                scanData.conn = conns.checkoutConnection(scanData.dbinfo, cred);
            } catch (Throwable iex) {
    ...
    private boolean scanHost(DBInstanceInfo dbinfo) {
        ...
        try {
            ...
            for (String mgName : mgNames) {
                ...
                if (!mg.isAuto()) { // skip groups requiring manual configuration for now
                    if (!this.frameworkContext.getMetricsDef().getUdmManager().isMetricsGroupSubscribed(dbinfo.getDbGroupName(), dbinfo.getHostName(), mg.getGroupName(), null))
                        continue;
                }
            ...
            // UDM
            List<String> udms = this.frameworkContext.getMetricsDef().getUdmManager().getMetricsSubscriptions().getSubscribedUDMs(dbinfo.getDbGroupName(), dbinfo.getHostName());
            if (udms != null) {
                for (String s : udms) {
                    UserDefinedMetrics udm = this.frameworkContext.getMetricsDef().getUdmManager().getUDMByName(s);
                    if (udm == null || !"SQL".equals(udm.getSource())) continue;
            ...
            // scan
            {
                ...
                if (ist != null) {
                    ...
                    if (ist.getLastAlertTime() != null && ist.getLastAlertEndTime() == null) {
                        ...
                        if ((!"IO".equalsIgnoreCase(ist.getLastAlertType()) && !"SLOW".equalsIgnoreCase(ist.getLastAlertType()) && !"REPLLAG".equalsIgnoreCase(ist.getLastAlertType()) && !"REPLDOWN".equalsIgnoreCase(ist.getLastAlertType()) && !"MAXCONNERR".equalsIgnoreCase(ist.getLastAlertType())) || ist.getLastUpdateTime() == ist.getLastAlertTime().getTime() || ist.getLastUpdateTime() - ist.getLastReportTime() >= InstanceStates.REPETA_ALERT_DELAY) {
