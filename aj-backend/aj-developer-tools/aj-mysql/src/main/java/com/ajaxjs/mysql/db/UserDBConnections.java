...
public class UserDBConnections {
	...
	private List<DBConnectionWrapper> connections = new java.util.ArrayList<>();
	...
	synchronized public DBConnectionWrapper checkoutConnection(String dbGroupName, String hostname) {
		for (DBConnectionWrapper conn : this.connections) {
			if (dbGroupName.equalsIgnoreCase(conn.getDb().getDbGroupName()) && hostname.equalsIgnoreCase(conn.getDb().getHostName()) && !conn.isInuse()) {
				conn.setInuse(true);
				return conn;
			}
		}
	...
	/**
	 * Because it is possible to wait for too long to create a new connection, we
	 * relax this method from synchronized and move the synchronized to required
	 * blocks.
	 * 
	 * @param dbinfo
	 * @param cred
	...
	public DBConnectionWrapper checkoutConnection(DBInstanceInfo dbinfo, DBCredential cred) {
		synchronized (this) {
			for (DBConnectionWrapper conn : this.connections) {
				if (dbinfo.getDbGroupName().equalsIgnoreCase(conn.getDb().getDbGroupName()) && dbinfo.getHostName().equalsIgnoreCase(conn.getDb().getHostName())
						&& !conn.isInuse()) {
					conn.setInuse(true);
					return conn;
				}
		...
		// Because it could take too long to create a new connection to one db, it is
		// better not to
	...
	/**
	 * For Oracle When a target DB is not directly accessible, try to use gv view to
	 * access through another instance. Find first one available
	 * 
	 * @param cluster
	 * @param cred
	 * @return
	...
	private DBConnectionWrapper createConnectionInternal(DBInstanceInfo dbinfo, DBCredential cred) throws SQLException {
	...
	public void closeIdle(String clusterName, String hostName) {
		...
		synchronized (this) {
			for (int i = this.connections.size() - 1; i >= 0; i--) {
				DBConnectionWrapper conn = this.connections.get(i);
				if (!clusterName.equalsIgnoreCase(conn.getDb().getDbGroupName()) || !hostName.equalsIgnoreCase(conn.getDb().getHostName()) || conn.isInuse())
					continue;
		...
		for (DBConnectionWrapper conn : idleConnections) {
			logger.fine("Closing connection to (" + conn.getAppUser() + ", " + conn.getDb() + "), created at " + conn.getCreateTime() + ", ignore inuse "
					+ conn.isInuse());
			conn.close();
			logger.fine("Closed connection to (" + conn.getAppUser() + ", " + conn.getDb() + "), created at " + conn.getCreateTime() + ", ignore inuse "
					+ conn.isInuse());
		}
	}
