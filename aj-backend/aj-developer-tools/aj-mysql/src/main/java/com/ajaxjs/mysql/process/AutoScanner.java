...
/**
 * This will automate AWRScanner and LongRunSqlScanner. The result can be
 * accessed from FrameworkContext
 * 
 * @author xrao
 *
 */
...
//	public static final char lineSeparator = '\n';
	...
	// store data. the first key is dbgroupname;dbhost, the second key is metrics
	// group name
	...
	private void startMetricScheduler() {
		...
		if (maxScanIdleTime < 300000L)
			maxScanIdleTime = 300000L;// minimum check time 5 minutes
		...
		// configDelay2 = 10;
		ScheduledFuture<?> metricsTaskFuture = metricsScheduler.scheduleAtFixedRate(metricsTask, metricsDelay, interval, TimeUnit.SECONDS);
		logger.info("AutoScanner started: runtime scan for each " + interval + " seconds with delay of  " + metricsDelay + " seconds, run as user "
	...
	private void startAlertScheduler() {
		...
		int alertDelay = (int) Math.ceil(((double) secOfTheDay) / (context.getMyperfConfig().getAlertScanIntervalSeconds()))
				* context.getMyperfConfig().getAlertScanIntervalSeconds() - secOfTheDay;
		ScheduledFuture<?> alertTaskFuture = alertScheduler.scheduleAtFixedRate(alertTask, alertDelay, context.getMyperfConfig().getAlertScanIntervalSeconds(),
				TimeUnit.SECONDS);
		logger.info("AlertScanner started: runtime scan for each " + context.getMyperfConfig().getAlertScanIntervalSeconds() + " seconds with delay of  "
				+ alertDelay + " seconds, run as user " + context.getMyperfConfig().getMetricsScannerUser());
	}
	...
	private void startConfigRetentionScheduler() {
		...
		ScheduledFuture<?> runtimeTaskFuture2 = scheduler2.scheduleAtFixedRate(retentionTask, configDelay2 + 60, 24 * 3600, TimeUnit.SECONDS);// once a day
		ScheduledFuture<?> runtimeTaskFuture3 = scheduler2.scheduleAtFixedRate(configScanTask, configDelay + 120, 12 * 3600, TimeUnit.SECONDS);// twice a day
	...
	private void startMonitorScheduler() {
		...
		ScheduledFuture<?> monitorTaskFuture = this.monitorScheduler.scheduleAtFixedRate(monitorTask, monitorDelay, 600, TimeUnit.SECONDS);// each 10 minutes
