...
public class ProcessListEntryAggregate implements java.io.Serializable, Comparable<ProcessListEntryAggregate> {
	...
	public static class SortByCount implements java.util.Comparator<ProcessListEntryAggregate> {
		...
		@Override
		public int compare(ProcessListEntryAggregate obj1, ProcessListEntryAggregate obj2) {
	...
	/**
	 * Record one execution instance time. Later we might want to add more metrics
	 * for example, rows_read, etc
	 *
	 * @param time_sec
	 */
	...
	public static String generateDigest(String str) {
		...
		try {
			...
			for (byte b : digest) {
				sb.append(HEXES[(b & 0xF0) >> 4]).append(HEXES[(b & 0x0F)]);
			}
			return sb.toString();
		} catch (Exception ex) {
