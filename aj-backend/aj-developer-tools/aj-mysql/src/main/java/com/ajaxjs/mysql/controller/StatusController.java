...
public class StatusController extends MyPerfBaseController {
		...
		if (dbhost == null || dbhost.isEmpty())// we should have id either as all or a dbgroup
		{
			...
			for (Map.Entry<String, DBGroupInfo> e : this.frameworkContext.getDbInfoManager().getClusters().entrySet()) {
				for (DBInstanceInfo dbinfo : e.getValue().getInstances()) {
					...
					if (stateSnaps == null)
						continue;
					// java.util.Date lastConnectTime =
					// this.frameworkContext.getDbInfoManager().getLastAccessDate(dbgroupName,
					// dbHostName);
					Date lastConnectTime = stateSnaps.getLastAccessTime();
					...
					if (isSnapValid) {
						...
						if (snaps[0].getQueries() >= 0L && snaps[1].getQueries() >= 0L) {
							// double val = ((double)(snaps[1].getQueries() -
							// snaps[0].getQueries())*1000)/(double)interval;
							double val = diffAvgNoNegative(snaps[1].getQueries(), snaps[0].getQueries(), snaps[1].getTimestamp(), snaps[0].getTimestamp(),
									snaps[1].getUptime(), true, 1000);
						...
						if (snaps[0].getSlowQueryCount() >= 0L && snaps[1].getSlowQueryCount() >= 0L) {
							double val = diffAvgNoNegative(snaps[1].getSlowQueryCount(), snaps[0].getSlowQueryCount(), snaps[1].getTimestamp(),
									snaps[0].getTimestamp(), snaps[1].getUptime(), true, 60000);
							vals.add(df.format(val));
						} else
							vals.add("0.00");
						...
						if (snaps[0].getConnections() >= 0L && snaps[1].getConnections() >= 0L) {
							// double val = ((double)(snaps[1].getConnections() -
							// snaps[0].getConnections())*1000)/(double)interval;
							double val = diffAvgNoNegative(snaps[1].getConnections(), snaps[0].getConnections(), snaps[1].getTimestamp(),
						...
						if (snaps[0].getAbortedConnectsClients() >= 0L && snaps[1].getAbortedConnectsClients() >= 0L) {
							// double val = ((double)(snaps[1].getAbortedConnectsClients() -
							// snaps[0].getAbortedConnectsClients())*1000)/(double)interval;
							double val = diffAvgNoNegative(snaps[1].getAbortedConnectsClients(), snaps[0].getAbortedConnectsClients(), snaps[1].getTimestamp(),
									snaps[0].getTimestamp(), snaps[1].getUptime(), true, 1000);
							vals.add(df.format(val));
						} else {
							vals.add("0.00");
						...
						if (snaps[0].getDeadlocks() >= 0L && snaps[1].getDeadlocks() >= 0L) {
							double val = diffAvgNoNegative(snaps[1].getDeadlocks(), snaps[0].getDeadlocks(), snaps[1].getTimestamp(), snaps[0].getTimestamp(),
									snaps[1].getUptime(), false, 1);
	...
	/**
	 * diff between v1 and v2. If diff is negative and uptime is positive, use v1.
	 * Otherwise 0. If avg is set, divide by (t1 - t2), or uptime*1000 for case
	 * v1<v2 , then multiply by adjustment.
	 *
	 * @param v1
	 * @param v2
	 * @param ts1
