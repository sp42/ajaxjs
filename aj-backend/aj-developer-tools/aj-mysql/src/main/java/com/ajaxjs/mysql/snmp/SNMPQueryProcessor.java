...
import com.ajaxjs.mysql.snmp.SNMPClient.SNMPTriple;
import com.ajaxjs.util.logger.LogHelper;

/**
 * Query a single category of SNMP data, use parameter p_1 to decide the
 * category sys disk network all (all of above) default to all
 *
 * @author xrao
 */
public class SNMPQueryProcessor implements CustomQueryProcessor {
	private static final LogHelper LOGGER = LogHelper.getLog(SNMPQueryProcessor.class);
	...
	public void queryMultiple(MyPerfContext context, DBInstanceInfo dbinfo, String appUser, DBConnectionWrapper connWrapper, QueryParameters qps,
			Map<String, ResultList> rListMap) {
	...
	public ResultList querySingle(MyPerfContext context, DBInstanceInfo dbinfo, String appUser, DBConnectionWrapper connWrapper, QueryParameters qps)
		...
		try {
			...
			client.setSnmpSetting(context.getSnmpSettings().getHostSetting(dbinfo.getDbGroupName(), dbinfo.getHostName()));
			client.start();
			...
			if ("disk".equalsIgnoreCase(cat)) {
				...
				if (res == null || res.getRows().size() == 0 || (res.getRows().size() == 1 && "noSuchObject".equals(res.getRows().get(0).getColumns().get(2))))
				...
				if (sysList == null)
					return null; // don't expect no system data
				try {
...
//					logger.log(Level.INFO, "Failed to query storage data", ex);
	...
	/**
	 * Query disk data, in (name, oid, value). Note name will be prefixed with disk
	 * name.
	 *
	 * @param qps
	 * @return
