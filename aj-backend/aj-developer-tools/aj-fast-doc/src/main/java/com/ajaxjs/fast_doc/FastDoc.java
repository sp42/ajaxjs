...
public class FastDoc {
    ...
    public static void loadControllerBeans(String dir, Class<?>... clazz) {
        Params params = new Params();
        params.sources = new ArrayList<>();
        ...
        for (Class<?> clz : clazz) {
            ...
            // 当前我们架构 Controller 的方法都在 Service，他们之间通过 继承 连接，所以
            Class<?>[] i = clz.getInterfaces();

            if (!ObjectUtils.isEmpty(i)) {
                for (Class<?> c : i) {
//                    params.sources.add(dir + Util.className2JavaFileName(c));
                }
            }
        }
    ...
    public static void loadControllersDoc(Class<? extends SpringMvcAnnotationParser> parserClz, String dir, Class<?>... clazz) {
        loadControllerBeans(dir, clazz);

        for (Class<?> clz : clazz) {
            if (clz.getAnnotation(IgnoreController.class) != null)
                continue;
...
//                TestHelper.printJson(controllerInfo);
    ...
    private static void mix(String fullName, ControllerInfo controllerInfo) {
        ...
        for (Item item : methodItems) {
            ...
            if (!ObjectUtils.isEmpty(javaDocInfo.methods))
