...
public class RequestParam {
    ...
    public static Object[] getArgs(IController controller, MvcRequest req, HttpServletResponse resp, Method method) {
        ...
        for (int i = 0; i < parmTypes.length; i++) {
            ...
            if (clazz.equals(HttpServletRequest.class) || clazz.equals(MvcRequest.class)) // 常见的 请求/响应 对象，需要的话传入之
                args.add(req);
            else if (clazz.equals(HttpServletResponse.class)) args.add(resp);
            else if (clazz.equals(HttpSession.class)) args.add(req.getSession());
            else if (clazz.equals(Map.class)) { // map 参数，将请求参数转为 map
                Map<String, Object> map;

                if (req.getMethod() != null && req.getMethod().equals("PUT")) map = req.getPutRequestData();
                else map = MapTool.as(req.getParameterMap(), arr -> MappingValue.toJavaValue(arr[0]));

                args.add(map);

                if (map.size() == 0) LOGGER.info("没有任何请求数据，但控制器方法期望至少一个参数来构成 map。");
            } else if (clazz.equals(ModelAndView.class)) args.add(new ModelAndView()); // 新建 ModeView 对象
                ...
                if (annotations != null && annotations.length >= 1 && annotations[0] instanceof SubBean) {
                    ...
                    args.add(bean);
                } else args.add(req.getBean(clazz));
    ...
    private static void getArgValue(Class<?> clz, Annotation[] annots, MvcRequest req, ArrayList<Object> args, Method method, IController controller) {
        if (annots.length > 0) {
            ...
            for (Annotation annot : annots) {
                if (annot instanceof NotNull) required = true;

                if (annot instanceof DefaultValue) defaultValue = ((DefaultValue) annot).value();
                ...
                if (annot instanceof QueryParam || annot instanceof FormParam || annot instanceof HeaderParam) {
                    ...
                    } else LOGGER.warning(new NullPointerException("控制器方法居然没有 PathParam 注解？？"));
        ...
        } else args.add("Nothing to add args"); // 不知道传什么，就这个字符串吧
    }
    ...
    private static String getArgValue(Annotation a, HttpServletRequest request, boolean required) {
        ...
        if (a instanceof QueryParam) key = ((QueryParam) a).value();
        else if (a instanceof FormParam) key = ((FormParam) a).value();
        else if (a instanceof HeaderParam) key = ((HeaderParam) a).value();
    ...
    private static void getArgValue(Class<?> clz, ArrayList<Object> args, String value, String defaultValue) {
        if (clz == String.class) args.add(value == null ? defaultValue : value);
        else if (clz == int.class || clz == Integer.class) {
            boolean isNull = value == null || "".equals(value) || "null".equals(value);
            int v = 0;
            if (isNull && defaultValue != null) v = Integer.parseInt(defaultValue);
            else if (!isNull) v = Integer.parseInt(value);

            args.add(v);
        } else if (clz == long.class || clz == Long.class)
            args.add(value == null || "".equals(value) ? 0L : (Long.parseLong(value)));
        else if (clz == boolean.class || clz == Boolean.class) args.add(MappingValue.toBoolean(value));
        else {
