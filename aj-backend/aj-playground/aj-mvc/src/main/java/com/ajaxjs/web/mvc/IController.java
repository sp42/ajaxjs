...
public interface IController {
     ...
     */
    Map<String, Action> urlMappingTree = new HashMap<>();
    ...
    static Action findTreeByPath(Map<String, Action> tree, Queue<String> path, String basePath, boolean createIfEmpty) {
        while (!path.isEmpty()) {
            ...
            if (path.isEmpty()) return target;// found it!
    ...
    static Action findTreeByPath(Map<String, Action> tree, Queue<String> queue, String basePath) {
        return findTreeByPath(tree, queue, basePath, false);
    }

    static Action findTreeByPath(Map<String, Action> tree, String path, String basePath, boolean createIfEmpty) {
        return findTreeByPath(tree, ListMap.split2Queue(path), basePath, createIfEmpty);
    }

    static Action findTreeByPath(Map<String, Action> tree, String path, String basePath) {
    ...
    static Action findTreeByPath(String path) {
        ...
        if (match.find()) path = match.replaceAll("/{id}");
...
//		}
    ...
    /**
     * Covert "aa/bb/cc" to queen.
     */
    static Queue<String> split2Queue2(String path) {
        String[] arr = path.split("/");

        if (arr.length == 1) arr = new String[]{"", arr[0]}; // for the case of the root
