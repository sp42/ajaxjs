...
public class ListMap {
        ...
        for (String key : map.keySet()) {
            ...
            if (value instanceof List || value instanceof Map) {
                ...
                if (value instanceof Map)
                    traveler((Map<String, Object>) value, firstCtx, map, level + 1, config);

                if (value instanceof List) {
                    List<?> list = (List<?>) value;

                    if (list.size() > 0 && (list.get(0) instanceof Map))
                        traveler((List<Map<String, Object>>) value, firstCtx, map, level + 1, config);
                }
    ...
    public static void traveler(List<Map<String, Object>> list, ListMapConfig.Context firstCtx, Map<String, Object> superMap, int level, ListMapConfig config) {
        for (Map<String, Object> map : list) {
            ...
            traveler(map, firstCtx, superMap, level, config);
        }
    }
