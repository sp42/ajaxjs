 ...
 */
public class SymmetricCipherInfo {
    /**
     * 加密算法
     */
     ...
     * @param keySize   密钥长度
     */
    public SymmetricCipherInfo(String algorithm, int keySize) {
     ...
     * @return 转换后的内容
     */
    static byte[] doCipher(SymmetricCipherInfo ci, int mode, String key, byte[] bytes) {
    ...
    public static String getSecretKey(String algorithm, SecureRandom secure) {
        return Utils.base64EncodeToString(Objects.requireNonNull(getSecretKey(algorithm, secure, null)).getEncoded());
    }
