 ...
 * 具体内容 需要关注 JDK Document <a href="http://.../docs/technotes/guides/security/SunProviders.html">...</a>
 * </pre>
 */
public class DES {
    /**
...
//				// 不带参数的初始化
            ...
            str = SymmetricCipherInfo.getSecretKey(KEY_DES, secure);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//            secretKey = new SecretKeySpec(key, KEY_DES);
    ...
    public static String decryptDES(String data, String key) {
        Key k = byteToKey(Utils.decodeFromString(key));
        byte[] bytes = SymmetricCipherInfo.doCipher(KEY_DES, Cipher.DECRYPT_MODE, k, null, hexString2Bytes(data));
    ...
    public static String encryptDES(String data, String key) {
        Key k = byteToKey(Utils.decodeFromString(key));
        byte[] bytes = SymmetricCipherInfo.doCipher(KEY_DES, Cipher.ENCRYPT_MODE, k, null, data.getBytes());

        assert bytes != null;
        return Utils.bytesToHexStr(bytes);// 将得到的字节数组变成字符串返回
    }
