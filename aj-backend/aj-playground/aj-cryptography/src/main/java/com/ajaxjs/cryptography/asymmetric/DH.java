package com.ajaxjs.cryptography.asymmetric;

import com.ajaxjs.cryptography.Utils;
import com.ajaxjs.cryptography.encryption.SymmetricCipherInfo;
...
public class DH extends Common {
    ...
    /**
     * DH 加密下需要一种对称加密算法对数据加密，这里我们使用 DES，也可以使用其他对称加密算法
     */
    ...
    public static Map<String, byte[]> init(String key) {
        try {
            // 解析甲方密钥
            PublicKey publicKey = KeyFactory.getInstance(KEY_DH).generatePublic(new X509EncodedKeySpec(Utils.decodeFromString(key)));
            // 由甲方公钥构建乙方密钥
     ...
     * @return 结果
     */
    private static byte[] doDH(boolean isEncrypt, byte[] data, String publicKey, String privateKey) {
        SecretKey secretKey = getSecretKey(publicKey, privateKey);// 生成本地密钥
        // 数据解密
        assert secretKey != null;
        return SymmetricCipherInfo.doCipher(secretKey.getAlgorithm(), isEncrypt ? Cipher.ENCRYPT_MODE : Cipher.DECRYPT_MODE, secretKey, null, data);
    }
    ...
    public static String getPrivateKey(Map<String, byte[]> keyPair) {
        return getKey(KEY_DH_PRIVATE_KEY, keyPair);
    }
    ...
    private static SecretKey getSecretKey(String publicKey, String privateKey) {
        try {
            ...
            // 初始化公钥
            PublicKey localPublicKey = factory.generatePublic(new X509EncodedKeySpec(Utils.decodeFromString(publicKey)));
            // 初始化私钥
            PrivateKey localPrivateKey = factory.generatePrivate(new PKCS8EncodedKeySpec(Utils.decodeFromString(privateKey)));
