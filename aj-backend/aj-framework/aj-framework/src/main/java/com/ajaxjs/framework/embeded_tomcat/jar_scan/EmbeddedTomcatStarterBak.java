 ...
 * 嵌入式使用 Tomcat
 */
public class EmbeddedTomcatStarterBak {
    private static final LogHelper LOGGER = LogHelper.getLog(com.ajaxjs.framework.spring.EmbeddedTomcatStarter.class);
...
//      filter.setTldSkip(newTldSkip);
    ...
    public static void start(int port, Class<?>... clz) {
        new EmbeddedTomcatStarterBak().init(port, clz);
    }
    ...
    public static void start(int port, String contextPath, Class<?>... clz) {
        new EmbeddedTomcatStarterBak().init(port, contextPath, clz);
    }
