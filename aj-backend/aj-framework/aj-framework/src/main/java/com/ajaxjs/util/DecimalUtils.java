...
public class DecimalUtils {
     ...
     *
     * @param x      被加数（可为 null）
     * @param result 和 （可为 null,若被加数不为为null，result默认值为0）
     ...
     * @param x 被除数（可为 null）
     * @param y 除数（可为 null）
     * @return 商 （可为 null,四舍五入，默认保留20位小数）
    ...
    public static BigDecimal divide(BigDecimal x, BigDecimal y) {
        ...
        return stripTrailingZeros(x.divide(y, 20, RoundingMode.HALF_UP));
    }
