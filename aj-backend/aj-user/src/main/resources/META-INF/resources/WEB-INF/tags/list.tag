...
<%@ tag pageEncoding="UTF-8" import="com.ajaxjs.user.admin.jsp.JspHelper"%>
...
<%@attribute fragment="false" required="false" name="show_create" type="Boolean" description="是否显示 新建 按钮"%>
<%@attribute fragment="false" required="false" name="page" type="Boolean" description="是否分页"%>
<%@attribute fragment="false" required="false" name="listData" type="java.util.List" description="列表数据"%>
<%
	JspHelper.getJspHelper(request);
	Object listData = jspContext.getAttribute("list");
...
<body class="middleHeight">
	...
	<script>
		...
		function del(id, name) {
			if (confirm('警告：\n是否删除记录 [' + (name || id) + "]？")) {
				jsp.xhr.del('${ctx}/data_service/${namespace}?id=' + id, j => {
					if(j && j.status === 1 && j.data === true) {
						alert('删除${namespace_chs}成功');
						location.reload();
