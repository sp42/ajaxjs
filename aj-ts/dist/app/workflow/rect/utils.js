    ...
    (function (svg) {
        var Utils = /** @class */ (function () {
            ...
            Utils.connPoint = function (rect, p) {
                var start = p, end = { x: rect.x + rect.width / 2, y: rect.y + rect.height / 2 }, tag = (end.y - start.y) / (end.x - start.x); // 计算正切角度
                tag = isNaN(tag) ? 0 : tag;
                var rectTag = rect.height / rect.width, 
                // 计算箭头位置
                xFlag = start.y < end.y ? -1 : 1, yFlag = start.x < end.x ? -1 : 1, arrowTop = 0, arrowLeft = 0;
                // 按角度判断箭头位置
            ...
            Utils.arrow = function (p1, p2, r) {
                var atan = Math.atan2(p1.y - p2.y, p2.x - p1.x) * (180 / Math.PI), centerX = p2.x - r * Math.cos(atan * (Math.PI / 180)), centerY = p2.y + r * Math.sin(atan * (Math.PI / 180)), x2 = centerX + r * Math.cos((atan + 120) * (Math.PI / 180)), y2 = centerY - r * Math.sin((atan + 120) * (Math.PI / 180)), x3 = centerX + r * Math.cos((atan + 240) * (Math.PI / 180)), y3 = centerY - r * Math.sin((atan + 240) * (Math.PI / 180));
            ...
            return new Vue({
                ...
                methods: {
                    ...
                    setXY_vBox: function (vBox) {
                        var w = vBox.x + vBox.width / 2, h = vBox.y + vBox.height / 2, textBox = this.$el.getBoundingClientRect();
