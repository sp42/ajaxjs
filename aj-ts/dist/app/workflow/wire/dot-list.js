        ...
        */
        var DotList = /** @class */ (function (_super) {
            ...
            function DotList(from, to) {
                ...
                var fromBB = from.getBBox(), toBB = to.getBBox(), fromPos = svg.Utils.connPoint(fromBB, { x: toBB.x + toBB.width / 2, y: toBB.y + toBB.height / 2 }), // 起点是 box 中央，这个是起点坐标
                toPos = svg.Utils.connPoint(toBB, fromPos), // 终点坐标
                /* 先创建中间点，一开始左右点都是 null 的，——这没关系，先空着，下面会计算 */
                midDot = new svg.Dot(svg.DOT_TYPE.SMALL, { x: (fromPos.x + toPos.x) / 2, y: (fromPos.y + toPos.y) / 2 }, null, null, _this); // 中间点
                /* 起点没有 left 点，right 点就是 midDot 了 */
                _this.fromDot = new svg.Dot(svg.DOT_TYPE.FROM, fromPos, null, midDot, _this); // 起点对象
                /* 终点没有 right 点，left 点就是 midDot 了 */
                _this.toDot = new svg.Dot(svg.DOT_TYPE.TO, toPos, midDot, null, _this); // 终点对象
                /* 补充中间点的左右两点信息 */
            ...
            DotList.prototype.toPathString = function () {
                if (!this.fromDot)
                    return "";
                var d = this.fromDot, p = "M" + d.pos().x + " " + d.pos().y, // 线的路径
                arr = [];
                // 线的路径
                while (d.right()) {
                    d = d.right();
                ...
                p += arr.join('');
                // 根据箭头三点的坐标生成 SVG 路径
                var arrPos = svg.Utils.arrow(d.left().pos(), d.pos(), 4), arrow = "M" + arrPos[0].x + " " + arrPos[0].y + "L" + arrPos[1].x + " " + arrPos[1].y + "L" + arrPos[2].x + " " + arrPos[2].y + "z";
                return [p, arrow];
            };
            /**
             * 遍历链表，查找中间的点
            ...
            DotList.prototype.toJson = function () {
                return svg.serialize.dotList.toJson(this.fromDot);
            };
            /**
             * 输入坐标数据，还原图形
             *
