...
namespace aj.svg {
    export class Utils {
        ...
        public static connPoint(rect: VBox, p: Point): Point {
            var start: Point = p, end: Point = { x: rect.x + rect.width / 2, y: rect.y + rect.height / 2 };
         ...
         * @param r  前头的边长
         */
        public static arrow(p1: Point, p2: Point, r: number) {
    ...
    // 创建文本节点，返回 vue 实例
    aj.svg.createTextNode = (_text, x: number, y: number) => {
        ...
        return new Vue({
            ...
            methods: {
                ...
                setXY(x: number, y: number) {
                    this.$el.setAttributeNS(null, "x", x); this.$el.setAttributeNS(null, "y", y);
                },
                getXY(): Point {
                    return { x: Number(this.$el.getAttribute('x')), y: Number(this.$el.getAttribute('y')) };
                },
                // 在一个 box 中居中定位文字
                setXY_vBox(vBox: VBox) {
