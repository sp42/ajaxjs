namespace aj.svg {
    ...
    export class Dot implements SimpleSharp {
         ...
         * @param rightDot  右边的点
         */
        constructor(type: DOT_TYPE, pos: Point, path: Path, leftDot?: Dot, rightDot?: Dot) {
            ...
            switch (this.type) {
                ...
                case DOT_TYPE.SMALL: // 移动小点时，转变为大点，增加俩个小点
                    if (_lt && _rt && !Utils.isLine(_lt.pos(), _pos, _rt.pos())) {
                        ...
                        let P = new Dot(DOT_TYPE.SMALL, Utils.center(_lt.pos(), _pos), path, _lt, _lt.right());
                        _lt.right(P);
                        this._leftDot = _lt = P;
                        let R = new Dot(DOT_TYPE.SMALL, Utils.center(_rt.pos(), _pos), path, _rt.left(), _rt);
                        _rt.left(R);
